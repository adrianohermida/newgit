...
export default function ProcessosResultadoView({ actionState, jobs, activeJobId, JobCard, OperationResult, executionHistory, remoteHistory, buildDrainPreview }) {
  ...
  return <div id="resultado" className="grid flex-1 auto-rows-fr items-stretch gap-6 2xl:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)]">
    ...
    />
    <OperationalHistoryCompactCard className="h-full" primaryText={executionHistory[0] ? `${executionHistory[0].label || executionHistory[0].action} â€¢ ${executionHistory[0].status}` : ""} secondaryLabel="Ultima leitura remota" secondaryText={remoteHistory[0] ? `${getProcessActionLabel(remoteHistory[0].acao, remoteHistory[0].payload || {})} â€¢ ${remoteHistory[0].status}` : ""} />
  </div>;
}
