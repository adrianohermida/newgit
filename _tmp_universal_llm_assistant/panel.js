...
async function initPanel() {
  ...
  el.tabSessions.addEventListener("click", async () => { closeOverlays(el); switchTab(el, "sessions"); await renderSessions(el, addSystemMessage, switchTab, addMessage, updateProviderBadge); });
  el.tabTasks.addEventListener("click", async () => { closeOverlays(el); switchTab(el, "tasks"); await renderTasks(el); });
  el.tabAutomations.addEventListener("click", async () => { closeOverlays(el); switchTab(el, "automations"); await renderAutomations(el, addSystemMessage, switchTab); });
  ...
  el.btnSaveSettings.addEventListener("click", async () => { await saveSettings(el); updateProviderBadge(el); addSystemMessage(el, "Configuracoes salvas."); await bootstrapBridge(el); closeOverlays(el); });
