...
export function collectElements() {
  return {
    ...
    inputProviderLabel: $("input-provider-label"),
    inputChatPath: $("input-chat-path"),
    inputExecutePath: $("input-execute-path"),
...
export function updateProviderBadge(el) {
  const fallbackMeta = PROVIDER_META[state.provider] || { label: state.provider, color: "#6b7280" };
  const meta = state.provider === "local"
    ? { ...fallbackMeta, label: state.settings.providerLabel || fallbackMeta.label }
    : fallbackMeta;
  const localOption = el.providerSelect?.querySelector('option[value="local"]');
  if (localOption) localOption.textContent = state.settings.providerLabel || PROVIDER_META.local.label;
