...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return jsonResponse(
      {
        success: false,
        error: "Supabase env ausente para persistência",
      },
      500,
    );
  }
  ...
  try {
    ...
    const runInsert = await supabase
      ...
      .insert({
        ...
        status: "running",
        metadata: {
          requested_filter_id: requestedFilterId || null,
        },
      })
      .select("id")
      .single();
    ...
    if (!selectedFilter?.id) {
      throw new Error("Nenhum filtro disponível para a entidade");
    }
    ...
    if (!recordsResponse.ok) {
      throw new Error(`Freshsales retornou ${recordsResponse.status} para ${entity}`);
    }
