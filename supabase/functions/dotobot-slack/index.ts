...
// ═══ LLM Hub inline (fonte: _shared/llm-hub.ts) ═══
/**
...
function createLLMHub(supabaseUrl?: string, serviceRoleKey?: string) {
...
type LLMHub = ReturnType<typeof createLLMHub>;
// ═══ Fim LLM Hub ═══
...
async function handleIaPerguntar(channel: string, userText: string, userId: string) {
  const SUPABASE_URL_LOCAL = Deno.env.get("SUPABASE_URL") || "";
  const SVC_KEY_LOCAL = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || "";

  // Buscar histórico de conversa
  let history: { role: string; content: string }[] = [];
  try {
    const histRes = await fetch(
      `${SUPABASE_URL_LOCAL}/rest/v1/dotobot_memory?channel_id=eq.${encodeURIComponent(channel)}&order=created_at.desc&limit=10`,
      { headers: { apikey: SVC_KEY_LOCAL, Authorization: `Bearer ${SVC_KEY_LOCAL}` } }
    );
    if (histRes.ok) {
    ...
    console.error("[dotobot-ia] erro ao buscar histórico:", e);
  }

  const messages: ChatMessage[] = [
    {
      role: "system",
      content: `Você é o Dotobot, assistente jurídico e operacional do escritório Hermida Maia.
Responda de forma direta, profissional e objetiva em português (PT-BR).
Você tem acesso a dados de processos, publicações, audiências e contatos do Freshsales.
Quando não souber algo, diga claramente e sugira como obter a informação.`,
    },
    ...history.map((h) => ({ role: h.role as "user" | "assistant", content: h.content })),
    { role: "user", content: userText },
  ];

  const hub = createLLMHub(SUPABASE_URL_LOCAL, SVC_KEY_LOCAL);
  let responseText = "";
  let providerUsed = "desconhecido";

  try {
    const result = await hub.runLLM(messages);
    responseText = result.content;
    providerUsed = result.provider;
  } catch (e: any) {
    console.error("[dotobot-ia] LLM Hub falhou:", e?.message);
    responseText = "⚠️ Não consegui processar sua pergunta no momento. Tente novamente em instantes.";
  }

  // Salvar no histórico
  try {
    await fetch(`${SUPABASE_URL_LOCAL}/rest/v1/dotobot_memory`, {
      method: "POST",
      headers: {
        apikey: SVC_KEY_LOCAL,
        Authorization: `Bearer ${SVC_KEY_LOCAL}`,
        "Content-Type": "application/json",
        Prefer: "return=minimal",
      },
      body: JSON.stringify([
        { channel_id: channel, role: "user", content: userText, user_id: userId },
        { channel_id: channel, role: "assistant", content: responseText, user_id: "dotobot" },
      ]),
    });
  } catch (e) {
    console.error("[dotobot-ia] erro ao salvar histórico:", e);
  }

  const label = llmProviderLabel(providerUsed);
  await postSlack(channel, `${responseText}\n\n_via ${label}_`);
}
