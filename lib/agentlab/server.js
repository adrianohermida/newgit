...
function shouldUseRemoteDashboard(localData, warnings) {
  const missingWarnings = warnings.filter((item) => String(item.source || "").startsWith("agentlab_") || item.source === "workspace_ai_agents" || item.source === "ai_agents" || item.source === "conversas" || item.source === "freshsales_sync_runs" || item.source === "freshsales_sync_snapshots");
...
export async function getAgentLabDashboard(env) {
  ...
  const finalWarnings =
    filteredWarnings.length > 6
      ? [
          {
            source: "agentlab_environment",
            message:
              "Este ambiente esta apontando para um Supabase com schema parcial do AgentLab. O painel entrou em modo degradado e usou fallbacks onde possivel.",
          },
        ]
      : filteredWarnings;
  ...
  return {
    ...
    governance: {
      ...
      quickReplies: AGENTLAB_DEFAULT_QUICK_REPLIES,
    ...
    rollout: {
      ...
      workflowLibrary: AGENTLAB_DEFAULT_WORKFLOW_LIBRARY,
      intents: AGENTLAB_DEFAULT_INTENTS,
      knowledgeSources: AGENTLAB_DEFAULT_KNOWLEDGE_SOURCES,
    },
