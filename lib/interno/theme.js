...
export function resolveStoredInternalThemePreference() {
  ...
  return sanitizeInternalThemePreference(window.localStorage.getItem(INTERNAL_THEME_STORAGE_KEY)) || "system";
}
...
export function resolveInitialInternalThemeState() {
  ...
  const preference = resolveStoredInternalThemePreference();
  return {
    preference,
    resolvedTheme: resolveInternalTheme(preference),
  };
}
