...
function loadRuntimeEnv(cwd = process.cwd(), baseEnv = process.env) {
...
function isLocalDesktopRuntime(env = process.env, cwd = process.cwd()) {
...
function getIntegrationKitMode(env = process.env, cwd = process.cwd()) {
  return isLocalDesktopRuntime(env, cwd) ? "local-ops" : "static-safe";
}

function getIntegrationKitCapabilities(env = process.env, cwd = process.cwd()) {
