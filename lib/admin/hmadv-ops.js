...
function sanitizeSimpleIds(values = []) {
  return [...new Set((values || []).map((value) => sanitizeSimpleId(value)).filter(Boolean))];
}
...
async function loadProcessesByIds(ids = []) {
  const cleanIds = sanitizeSimpleIds(ids);
...
async function loadPublicacoesByProcessIds(processIds = [], limit = 12) {
  const ids = sanitizeSimpleIds(processIds);
...
export async function saveProcessRelation(payload = {}) {
  const payloadId = sanitizeSimpleId(payload.id);
  ...
  let rows = [];
  if (payload.id) {
    if (!payloadId) throw new Error("Identificador de relacao invalido para atualizacao.");
    rows = await hmadvRest(`processo_relacoes?id=eq.${payloadId}&select=id,numero_cnj_pai,numero_cnj_filho,tipo_relacao`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
...
export async function deleteProcessRelation(id) {
  const cleanId = sanitizeSimpleId(id);
