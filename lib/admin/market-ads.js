...
export async function getMarketAdsDashboardData() {
  const [draftRows, campaignRows, complianceRows, adRows, abRows, templateRows, integrations] = await Promise.all([
  ...
  const templateLibrary = mergeTemplateLibraries(buildTemplateLibrary(creativeRanking), persistedTemplates);
  return {
    ...base,
