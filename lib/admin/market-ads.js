...
function buildCampaignRevenueDiagnostics(campaigns = [], attributions = []) {
  ...
  return campaigns.map((campaign) => {
    ...
    const clients = items.filter((item) => item.stage === "cliente").length;
...
function buildLeadForecast(attributions = [], campaigns = [], adItems = []) {
  ...
  const totals = {
    total: queue.length,
    hot: queue.filter((item) => item.temperature === "quente" && item.stage !== "cliente").length,
    warm: queue.filter((item) => item.temperature === "morno" && item.stage !== "cliente").length,
    cold: queue.filter((item) => item.temperature === "frio" && item.stage !== "cliente").length,
    clients: queue.filter((item) => item.stage === "cliente").length,
  };
  ...
  if (queue.some((item) => item.ageInDays !== null && item.ageInDays >= 4 && item.stage !== "cliente")) {
    bottlenecks.push("Ha leads esfriando por tempo sem atualizacao. Reative follow-ups antes de abrir nova escala.");
  }
  if (queue.some((item) => !item.hasContact && item.stage !== "cliente")) {
