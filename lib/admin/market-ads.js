...
export async function getMarketAdsDashboardData() {
  const [draftRows, campaignRows, complianceRows, adRows, abRows, templateRows, templateUsageRows, integrations] = await Promise.all([
  ...
  const funnel = await buildFunnelOverview(effectiveCampaigns);
...
export async function toggleMarketAdsTemplateFavorite(templateId, isFavorite = true) {
...
export async function updateMarketAdsTemplateVisibility(templateId, visibility = "privado") {
  const id = String(templateId || "").trim();
  if (!id) {
