...
const ADMIN_RUNTIME_UNAVAILABLE_CACHE_TTL_MS = 180_000;
...
function buildAdminPathCandidates(path) {
  ...
  if (!isChatAdminRuntimePath(normalized) && canUseFunctionsAdminFallback()) {
    candidates.push(normalized.replace(/^\/api\//, "/functions/api/"));
  }

  return [...new Set(candidates.filter(Boolean))];
}

function isChatAdminRuntimePath(path) {
  const normalized = normalizeAdminPath(path);
  return normalized === "/api/admin-lawdesk-chat" || normalized === "/api/admin-dotobot-chat";
}
...
export async function adminFetch(path, init = {}, options = {}) {
  ...
  const requestedMaxRetries = Number.isInteger(options?.maxRetries) && options.maxRetries >= 0
    ? options.maxRetries
    : DEFAULT_FETCH_MAX_RETRIES;
  const maxRetries = isChatAdminRuntimePath(path)
    ? Math.min(requestedMaxRetries, 0)
    : requestedMaxRetries;
  const method = String(init?.method || "GET").toUpperCase();
  ...
  const allowFailurePayload = options?.allowFailurePayload === true;
  if (shouldCacheGet) {
      ...
      try {
        ...
        for (let pathIndex = 0; pathIndex < pathCandidates.length; pathIndex += 1) {
          ...
          try {
            ...
            const hasFailurePayload = response.ok && payload?.ok === false;
            if (hasFailurePayload && allowFailurePayload) {
              if (shouldCacheGet) {
