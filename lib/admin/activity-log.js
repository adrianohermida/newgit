...
const STORAGE_FINGERPRINT_STATE_KEY = "hmadv.activity_log.fingerprint_state";
...
function safeSetItem(storage, key, data) {
  ...
  try {
    ...
    try {
      const trimmed = Array.isArray(data) ? data.slice(-Math.ceil(data.length / 2)) : data;
      storage.setItem(key, JSON.stringify(trimmed));
    } catch {
      // quota ainda insuficiente — silent fail
    }
  }
}
...
function deriveExplicitTags(entry) {
  const tags = new Set();
  [
    ...(entry?.consoleTags || []),
    ...(entry?.domains || []),
  ...
  toTagList(entry?.consolePane).forEach((tag) => tags.add(tag));
  toTagList(entry?.consolePanes).forEach((tag) => tags.add(tag));
  toTagList(entry?.system).forEach((tag) => tags.add(tag));
  toTagList(entry?.domain).forEach((tag) => tags.add(tag));
  toTagList(entry?.channel).forEach((tag) => tags.add(tag));
...
function deriveDiagnosticTags(entry) {
  ...
  const pathName = normalizeText(entry?.path || entry?.page);
  const actionName = normalizeText(entry?.action);
  const statusName = normalizeText(entry?.status);
  ...
  if (
    ...
    haystack.includes("service_role") ||
    haystack.includes("service role") ||
    haystack.includes("rls") ||
    actionName.includes("security")
  ) {
    tags.add("security");
  }
  if (
    haystack.includes("/functions/") ||
    haystack.includes("functions/v1") ||
    haystack.includes("edge function") ||
    haystack.includes("edge-function") ||
    haystack.includes("serverless") ||
    haystack.includes("fs-webhook") ||
    haystack.includes("sync-worker") ||
    haystack.includes("datajud-worker") ||
    haystack.includes("processo-sync") ||
    haystack.includes("tpu-sync") ||
    actionName.includes("function") ||
    moduleName.includes("agentlab")
  ) {
    tags.add("functions");
  }
  if (
    ...
    haystack.includes("405") ||
    haystack.includes("not found") ||
    haystack.includes("method not allowed") ||
    pathName.startsWith("/api/") ||
    actionName.includes("route")
  ) {
    tags.add("routes");
  }
  if (
    ...
    actionName.includes("job")
  ) {
    tags.add("jobs");
  }
  if (haystack.includes("webhook") || actionName.includes("webhook")) {
    tags.add("webhook");
  }
  if (
    haystack.includes("freshsales") ||
    haystack.includes("crm/sales") ||
    haystack.includes("account_id_freshsales") ||
    haystack.includes("contact_id_freshsales") ||
    haystack.includes("sales activity") ||
    haystack.includes("sales_account") ||
    haystack.includes("contact") ||
    moduleName.includes("contacts") ||
    moduleName.includes("clientes") ||
    moduleName.includes("leads") ||
    moduleName.includes("financeiro")
  ) {
    tags.add("crm");
  }
  if (
    haystack.includes("supabase") ||
    haystack.includes("postgrest") ||
    haystack.includes("pgrst") ||
    haystack.includes("schema cache") ||
    haystack.includes("storage") ||
    haystack.includes("realtime") ||
    actionName.includes("supabase")
  ) {
    tags.add("supabase");
  }
  if (haystack.includes("dotobot") || moduleName.includes("dotobot")) {
    tags.add("dotobot");
  }
  if (
    haystack.includes("ai task") ||
    haystack.includes("admin-lawdesk-chat") ||
    haystack.includes("lawdesk-chat") ||
    haystack.includes("active run") ||
    haystack.includes("automation") ||
    moduleName.includes("ai-task")
  ) {
    tags.add("ai-task");
  }
  if (
    haystack.includes("duplicad") ||
    haystack.includes("sem contato") ||
    haystack.includes("match ambiguo") ||
    haystack.includes("qualidade") ||
    haystack.includes("cpf") ||
    haystack.includes("cnpj") ||
    haystack.includes("email") ||
    haystack.includes("telefone") ||
    haystack.includes("cep") ||
    actionName.includes("dedup") ||
    actionName.includes("quality")
  ) {
  ...
  if (statusName === "error" || statusName === "failed") {
    if (tags.has("supabase") || haystack.includes("pgrst")) {
      tags.add("schema");
    }
  }
...
function extractTraceHints(entry) {
  const text = [
    entry?.path,
    entry?.request,
    entry?.response,
  ...
  const hints = new Map();
  const patterns = [
    { type: "job", regex: /(?:job[_\s-]?id|fila)\D+([a-z0-9_-]{4,})/ig },
    { type: "run", regex: /(?:run[_\s-]?id|activeRun|run)\D+([a-z0-9_-]{4,})/ig },
    { type: "contact", regex: /(?:contactId|contact_id|freshsales_contact_id)\D+([a-z0-9_-]{3,})/ig },
    { type: "processo", regex: /(?:processo_id|processId)\D+([a-z0-9_-]{3,})/ig },
    { type: "account", regex: /(?:account_id|accountId|account_id_freshsales)\D+([a-z0-9_-]{3,})/ig },
  ];
  for (const pattern of patterns) {
    ...
    while ((match = pattern.regex.exec(text))) {
      ...
      if (!hints.has(key)) {
        hints.set(key, { type: pattern.type, value, label: `${pattern.type}:${value}` });
      }
    }
  }
...
function deriveEntryInsights(entry, tags = []) {
  ...
  const traceHints = extractTraceHints(entry);
  return {
    severity,
...
function deriveTags(entry) {
  ...
  deriveExplicitTags(entry).forEach((tag) => tags.add(tag));
...
function hydrateFromStorage() {
  ...
  const storedFilters = safeJsonParse(storage.getItem(STORAGE_FILTERS_KEY) || "{}", {});
  ...
  const storedFingerprintStates = safeJsonParse(storage.getItem(STORAGE_FINGERPRINT_STATE_KEY) || "{}", {});
  if (storedFingerprintStates && typeof storedFingerprintStates === "object") fingerprintStates = storedFingerprintStates;
}

function persistToStorage() {
  const storage = getStorage();
  if (!storage) return;
  safeSetItem(storage, STORAGE_KEY, logEntries);
  safeSetItem(storage, STORAGE_ARCHIVE_KEY, archivedLogs);
  safeSetItem(storage, STORAGE_NOTES_KEY, operationalNotes);
  if (persistedFilters && Object.keys(persistedFilters).length) {
    safeSetItem(storage, STORAGE_FILTERS_KEY, persistedFilters);
  } else {
    storage.removeItem(STORAGE_FILTERS_KEY);
  }
  safeSetItem(storage, STORAGE_FRONTEND_ISSUES_KEY, frontendIssues);
  safeSetItem(storage, STORAGE_SCHEMA_ISSUES_KEY, schemaIssues);
  safeSetItem(storage, STORAGE_MODULE_HISTORY_KEY, moduleHistory);
  safeSetItem(storage, STORAGE_FINGERPRINT_STATE_KEY, fingerprintStates);
}
...
export function appendActivityLog(entry) {
  ...
  enriched.traceHints = Array.isArray(enriched.traceHints) && enriched.traceHints.length ? enriched.traceHints : insights.traceHints;
  if (schemaIssue) {
...
export function updateActivityLog(entryId, patch = {}) {
  ...
  logEntries = logEntries.map((entry) => {
    ...
    merged.traceHints = Array.isArray(merged.traceHints) && merged.traceHints.length ? merged.traceHints : insights.traceHints;
    if (schemaIssue) {
      pushSchemaIssue(schemaIssue, "log");
    }
    return merged;
...
export function getFingerprintStates() {
  hydrateFromStorage();
  return { ...fingerprintStates };
}

export function setFingerprintState(fingerprint, patch = {}) {
  hydrateFromStorage();
  const key = String(fingerprint || "").trim();
  if (!key) return null;
  const current = fingerprintStates?.[key] || {};
  const next = {
    fingerprint: key,
    status: patch.status || current.status || "aberto",
    note: patch.note !== undefined ? String(patch.note || "").trim() : current.note || "",
    updatedAt: new Date().toISOString(),
    lastEntryId: patch.lastEntryId || current.lastEntryId || null,
    lastLabel: patch.lastLabel || current.lastLabel || null,
    source: patch.source || current.source || "console",
  };
  fingerprintStates = {
    ...fingerprintStates,
    [key]: next,
...
export function setActivityLogFilters(filters = {}) {
  hydrateFromStorage();
  persistedFilters = Object.fromEntries(
