...
export function getSupabaseEdgeConfig(env = {}) {
  ...
  const sharedSecret = resolveEnvValue(env, ["HMDAV_AI_SHARED_SECRET", "HMADV_AI_SHARED_SECRET", "LAWDESK_AI_SHARED_SECRET"]);
  return {
    baseUrl: explicit.value || (supabaseUrl.value ? `${supabaseUrl.value.replace(/\/+$/, "")}/functions/v1/${functionName.value}` : null),
...
async function probePrimaryExecute(baseUrl, sharedSecret = null) {
  ...
  for (const candidate of candidates) {
    try {
      ...
      const message = payload?.error || payload?.message || `Execution HTTP ${response.status}.`;
      errors.push(`[${candidate}] ${message}`);
    } catch (error) {
      errors.push(`[${candidate}] ${error?.message || "Execution probe failed."}`);
    }
  }
...
async function probePrimaryBackend(env = {}) {
  ...
  try {
    ...
    return {
      id: "gpt",
      status: response.ok && !executeFailed ? "operational" : "degraded",
      ...
      reason: response.ok
        ? executeFailed
          ...
          : "Backend principal respondeu ao healthcheck e a sonda de execucao."
