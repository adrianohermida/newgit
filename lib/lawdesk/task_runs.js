...
async function loadRunFromSupabase(env, runId, options = {}) {
  ...
  return {
    ...
    result: run.result || null,
    error: run.error || null,
    events_total: Number.isFinite(Number(eventsTotal)) ? Number(eventsTotal) : Array.isArray(events) ? events.length : null,
...
function getEventsSince(events = [], sinceEventId = "", sinceSequence = null) {
...
export async function getTaskRun(env, payload = {}) {
  ...
  let eventWindow = cachedRun ? getEventsSince(run.events || [], sinceEventId, sinceSequence) : {
    events: run.events || [],
  ...
  if (
    ...
    while (Date.now() < deadline) {
      ...
      if (latestFromStore) {
        run = latestFromStore;
        eventWindow = getEventsSince(run.events || [], sinceEventId, sinceSequence);
      } else {
