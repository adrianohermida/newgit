...
export function buildOfflineHealthSnapshot({ localStackSummary = null, ragHealth = null } = {}) {
  ...
  const supabaseConfigured = Boolean(supabase.baseUrlConfigured && supabase.serviceKeyConfigured);
  const supabaseReady = Boolean(supabaseConfigured && (supabaseEmbedding.ok || ragHealth?.signals?.supabaseAuthMismatch === false));
  ...
  return {
    ...
    items: [
      ...
      },
      {
        id: "supabase",
        label: "Persistência",
        value: supabaseReady ? "Supabase ativo" : supabaseConfigured ? "Supabase parcial" : "Obsidian somente",
        tone: normalizeTone(supabaseReady, supabaseConfigured),
        detail: supabaseEmbedding.error || supabase.baseUrlSource || null,
      },
      {
        id: "network",
        label: "Rede externa",
