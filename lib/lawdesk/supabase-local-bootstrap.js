...
function isOfflineObsidianPrimary({ localStackSummary = null, ragHealth = null } = {}) {
  const offlineMode = Boolean(localStackSummary?.offlineMode);
  const obsidian = ragHealth?.report?.obsidian || {};
  return Boolean(offlineMode && obsidian.vaultPathConfigured);
}
...
function buildReadinessChecklist({ localStackSummary = null, ragHealth = null, configured = false, ready = false } = {}) {
  ...
  const offlineObsidianPrimary = isOfflineObsidianPrimary({ localStackSummary, ragHealth });
  ...
  const checks = [
    ...
    },
    {
      ...
      ready: Boolean(supabaseEmbedding.ok || offlineObsidianPrimary),
      detail: offlineObsidianPrimary
        ? "Modo offline com Obsidian local ativo; embedding Supabase tratado como opcional."
        : supabaseEmbedding.ok
...
export function buildSupabaseLocalBootstrap({ localStackSummary = null, ragHealth = null } = {}) {
  ...
  const offlineObsidianPrimary = isOfflineObsidianPrimary({ localStackSummary, ragHealth });
  const ready = Boolean(configured && supabaseQuery.ok && (supabaseEmbedding.ok || offlineObsidianPrimary));
  ...
  const summaryMap = {
    ...
    supabase_local_ready: {
      ...
      detail: offlineObsidianPrimary
        ? "Persistencia estruturada local confirmada para sessoes e memoria, com Obsidian como RAG primario offline."
        : "Persistencia estruturada local confirmada para sessoes, memoria e embeddings.",
    },
    supabase_local_partial: {
      label: "Supabase local parcial",
      detail: offlineObsidianPrimary
        ? "O endpoint local existe; em offline o Obsidian ja cobre o RAG primario e o embedding Supabase fica opcional."
        : "O endpoint local existe, mas embeddings, query ou upsert ainda precisam fechar o circuito.",
    },
    ...
    schema: [
      {
        ...
        detail: "Cria tabela, HNSW e upsert da memória Dotobot com vetor 768.",
      },
      {
