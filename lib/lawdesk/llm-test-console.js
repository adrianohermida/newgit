...
export function inferLlmTestRecommendations(errorMessage = "", context = {}) {
  ...
  if (errorType === "missing_function") {
    ...
    recommendations.push("Revise o endpoint chamado por /api/admin-lawdesk-chat e o provider gpt para confirmar se a funcao de execucao ainda esta registrada.");
  }
  if (errorType === "configuration") {
    recommendations.push("Confirme se o provider selecionado esta configurado com aseUrl, secrets e model no servidor.");
    ...
    if (selectedProvider === "gpt") {
      recommendations.push("Para gpt, confirme PROCESS_AI_BASE ou LAWDESK_AI_BASE_URL e verifique se o backend expoe /execute ou /v1/execute.");
    }
  }
  ...
  if (errorType === "timeout") {
    recommendations.push("Meca latencia do provider e reduza carga/contexto para diferenciar timeout de indisponibilidade.");
  }
  ...
  if (providerEntry?.status && providerEntry.status !== "operational") {
    recommendations.push(O provider  ja aparece como  no healthcheck. Corrija essa camada antes de insistir no smoke test.);
  }
...
export function buildTechnicalDebugger({
  ...
  return buildDiagnosticReport({
    title: "Debugger tecnico completo",
    summary: Falha classificada como .,
    sections: [
      {
        label: "diagnostico",
        value: {
  ...
  return buildDiagnosticReport({
    title: "Debugger t̩cnico completo",
