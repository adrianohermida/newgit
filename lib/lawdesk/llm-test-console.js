...
export function classifyLlmTestError(message = "") {
  ...
  if (text.includes("worker threw exception") || text.includes("cloudflare worker exception") || text.includes("ray id:")) return "execution";
  if (
    text.includes("auth") ||
    text.includes("unauthorized") ||
    text.includes("forbidden") ||
    text.includes("secret") ||
    text.includes("token administrativo") ||
    text.includes("sessao administrativa") ||
    text.includes("perfil administrativo")
...
export function inferLlmTestRecommendations(errorMessage = "", context = {}) {
  ...
  if (errorType === "configuration") {
    ...
    if (selectedProvider === "local") {
      recommendations.push("Para `local`, valide `LOCAL_LLM_BASE_URL`, `LLM_BASE_URL`, `LAWDESK_CODE_API_BASE_URL`, `AICORE_API_BASE_URL` ou `DOTOBOT_PYTHON_API_BASE`, e revise `LOCAL_LLM_MODEL`, `LOCAL_LLM_API_KEY` e `LOCAL_LLM_AUTH_TOKEN`.");
    }
  ...
  if (errorType === "execution" && /worker threw exception|cloudflare worker exception|ray id:/i.test(errorMessage)) {
    recommendations.push("O worker remoto publicou excecao 1101. Revise os logs do Cloudflare usando o Ray ID exibido no erro.");
    recommendations.push("Confirme se o deploy mais recente do worker HMADV IA foi publicado com a rota `/v1/messages` e com a env `AETHERLAB_LEGAL_MODEL`.");
  }
...
export function buildTechnicalDebugger({
  ...
  const expectedEnvByProvider = {
    ...
    local: ["LOCAL_LLM_BASE_URL", "LLM_BASE_URL", "LAWDESK_CODE_API_BASE_URL", "AICORE_API_BASE_URL", "DOTOBOT_PYTHON_API_BASE", "LOCAL_LLM_MODEL", "LOCAL_LLM_API_KEY", "LOCAL_LLM_AUTH_TOKEN"],
...
export function buildProviderDebugMatrix({
  ...
  return (Array.isArray(providerCatalog) ? providerCatalog : []).map((provider) => {
    ...
    return {
      ...
      catalogLoaded: providersHealth?.loaded !== false && Array.isArray(providerCatalog) && providerCatalog.length > 0,
...
export function buildLlmTestResultRecord({
  ...
  return {
    ...
    requestedModel: responseData?._metadata?.requestedModel || responseData?._metadata?.model || null,
    resolvedModel: responseData?._metadata?.resolvedModel || null,
...
export function buildLlmTestTimeline(result = {}) {
  ...
  if (result.model) {
    timeline.push({ tone: "info", label: "Modelo solicitado", value: result.requestedModel || result.model });
  }
  if (result.resolvedModel && result.resolvedModel !== result.model) {
    timeline.push({ tone: "info", label: "Engine real", value: result.resolvedModel });
  }
