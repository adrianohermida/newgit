...
export function buildLocalBootstrapPlan({ localStackSummary = null, ragHealth = null } = {}) {
  ...
  const steps = [
    ...
    },
    {
      ...
      done: Boolean(supabaseBootstrap.ready || supabaseBootstrap.mode === "obsidian_only"),
      optional: true,
      detail: supabaseBootstrap.configured
        ? `${supabaseBootstrap.label}. ${supabaseBootstrap.detail}`
        : "Opcional: configure Supabase local para sessoes, logs e embeddings. Sem isso, o bootstrap minimo opera com Obsidian.",
      action: supabaseBootstrap.configured ? "abrir_diagnostico" : "open_environment",
    },
  ];
