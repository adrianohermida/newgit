...
function getSupabaseEmbeddingModel(env) {
  return getClean(env.DOTOBOT_SUPABASE_EMBEDDING_MODEL) || "gte-small";
}
...
export async function runDotobotRagHealth(
  ...
  if (!config.enabled && !supabaseConfig.enabled) {
    return {
      ok: false,
      report,
      error:
        "RAG Dotobot nao habilitado. Configure CLOUDFLARE_WORKER_ACCOUNT_ID, CLOUDFLARE_WORKER_API_TOKEN e DOTOBOT_VECTORIZE_INDEX/CLOUDFLARE_VECTORIZE_INDEX, ou SUPABASE_URL/SUPABASE_SERVICE_ROLE_KEY com o embed function dotobot-embed.",
    };
  }
