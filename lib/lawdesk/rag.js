  ...
  try {
    if (config.enabled) {
      try {
        ...
        const isAuthError = error?.status === 401 || error?.status === 403 || /Authentication error/i.test(message);
    ...
    if (obsidianConfig.enabled) {
      try {
        ...
        report.obsidian = {
          ...obsidianConfig,
          ok: false,
          error: error?.message || "Falha no healthcheck do Obsidian.",
        };
      }
    }
  ...
  try {
    ...
    if (canUseSupabaseRag(env)) {
      try {
        ...
        trace.push({ source: "supabase", status: "ok" });
        queryUsed = true;
      } catch (error) {
        errors.push(error?.message || "Falha ao recuperar memoria RAG no Supabase.");
        trace.push({ source: "supabase", status: "error", error: error?.message || "unknown_error" });
      }
    }
    ...
    if (matches.length < topK && canUseObsidian(env)) {
      try {
        ...
        if (obsidian) {
          ...
          trace.push({ source: "obsidian", status: "ok" });
          queryUsed = true;
        }
      } catch (error) {
        errors.push(error?.message || "Falha ao recuperar memoria RAG no Obsidian.");
        trace.push({ source: "obsidian", status: "error", error: error?.message || "unknown_error" });
      }
    }
    ...
    if (matches.length < topK && canUseRag(env)) {
      try {
        ...
        trace.push({ source: "cloudflare", status: "ok" });
        queryUsed = true;
      } catch (error) {
        errors.push(error?.message || "Falha ao recuperar memoria RAG no Cloudflare.");
        trace.push({ source: "cloudflare", status: "error", error: error?.message || "unknown_error" });
      }
    }
    ...
    return {
      ...
      trace: [{ source: "rag", status: "error", error: error?.message || "unknown_error" }],
    };
  }
}
  ...
  try {
    ...
    if (canUseSupabaseRag(env)) {
      try {
        ...
        if (supabaseWrite.stored) {
          ...
          trace.push({ target: "supabase", status: "stored" });
        } else if (supabaseWrite.error) {
          errors.push(supabaseWrite.error);
          trace.push({ target: "supabase", status: "error", error: supabaseWrite.error });
        }
      } catch (error) {
        errors.push(error?.message || "Falha ao persistir memoria no Supabase.");
        trace.push({ target: "supabase", status: "error", error: error?.message || "unknown_error" });
      }
    }
    ...
    if (result.supabase.stored && canUseRag(env)) {
      try {
        ...
        trace.push({ target: "cloudflare", status: "stored" });
      } catch (error) {
        errors.push(error?.message || "Falha ao persistir memoria no Cloudflare.");
        trace.push({ target: "cloudflare", status: "error", error: error?.message || "unknown_error" });
      }
    }
      ...
      try {
        ...
        if (obsidian) {
          ...
          if (obsidianWrite.stored) {
            ...
            trace.push({ target: "obsidian", status: "stored", path: obsidianWrite.path });
          } else if (obsidianWrite.error) {
            errors.push(obsidianWrite.error);
            trace.push({ target: "obsidian", status: "error", error: obsidianWrite.error });
          }
        }
      } catch (error) {
        errors.push(error?.message || "Falha ao persistir memoria no Obsidian.");
        trace.push({ target: "obsidian", status: "error", error: error?.message || "unknown_error" });
      }
    }
    ...
    return {
      ...
      trace: [{ target: "rag", status: "error", error: error?.message || "unknown_error" }],
    };
  }
}
