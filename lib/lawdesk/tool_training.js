...
export function buildDotobotToolTrainingPrompt(repositoryContext = {}) {
  ...
  const externalAppsCoverage = Array.isArray(repositoryContext?.externalAppsCoverage)
    ...
    : [];
  const edgeFunctionExecutionMatrix = Array.isArray(repositoryContext?.edgeFunctionExecutionMatrix)
    ? repositoryContext.edgeFunctionExecutionMatrix
    : [];
  const edgeFunctionExecutionSummary =
    repositoryContext?.edgeFunctionExecutionSummary && typeof repositoryContext.edgeFunctionExecutionSummary === "object"
      ...
      : null;
  const routedUserFacingFunctions = edgeFunctionExecutionMatrix
    .filter((item) => item?.class === "user_facing" && item?.dispatcherStatus === "routed")
    .map((item) => item.name);
  const routedAdminFunctions = edgeFunctionExecutionMatrix
    .filter((item) => item?.class === "admin" && item?.dispatcherStatus === "routed_admin")
    .map((item) => item.name);
  ...
  return [
    ...
    edgeFunctionExecutionSummary
      ...
      : null,
    routedUserFacingFunctions.length
      ? `Funcoes HMADV ja roteadas de forma deterministica para o usuario final: ${routedUserFacingFunctions.join(", ")}.`
      : null,
    routedAdminFunctions.length
      ? `Funcoes HMADV roteadas apenas para contexto administrativo: ${routedAdminFunctions.join(", ")}.`
      : null,
    routedUserFacingFunctions.length
      ? "Quando a solicitacao combinar com uma dessas funcoes roteadas, priorize o dispatcher e nao trate a resposta como tarefa generativa aberta."
      : null,
