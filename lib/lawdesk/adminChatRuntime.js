...
export async function resolveAdminChatResponse(env, requestBody) {
  ...
  if (action) {
    ...
    const explicitStatus = Number(taskRunResult?.status);
    const shouldKeepHttpErrorStatus = Number.isFinite(explicitStatus) && explicitStatus >= 400 && explicitStatus < 500;
    return {
      status: shouldKeepHttpErrorStatus ? explicitStatus : 200,
