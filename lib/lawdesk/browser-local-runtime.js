...
export async function hydrateBrowserLocalProviderOptions(providerOptions = []) {
  ...
  try {
    ...
    const runtimeMeta = safeUrlMeta(joinRuntimeUrl(getBrowserLocalRuntimeConfig().runtimeBaseUrl, "/v1/messages"));
    ...
    nextOptions[localIndex] = {
      ...
      available: true,
      configured: true,
      model: localProvider?.model || previous.model || DEFAULT_LOCAL_MODEL,
      status: "operational",
...
export async function probeBrowserLocalStackSummary() {
  ...
  return {
    ...
    localProvider: {
      ...
      model: localProvider?.model || DEFAULT_LOCAL_MODEL,
