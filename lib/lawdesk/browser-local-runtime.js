...
const LOCAL_RUNTIME_OPT_IN_KEY = "hmadv_local_runtime_opt_in";
...
const EXTENSION_HEALTH_SUCCESS_CACHE_MS = 60 * 1000;
const EXTENSION_HEALTH_FAILURE_CACHE_MS = 10 * 60 * 1000;
const RUNTIME_HEALTH_SUCCESS_CACHE_MS = 60 * 1000;
const RUNTIME_HEALTH_FAILURE_CACHE_MS = 10 * 60 * 1000;
...
function buildBrowserLocalRuntimeDisabledError(message) {
  const error = new Error(message || "Runtime local do navegador desabilitado neste host.");
  error.code = "browser_local_runtime_disabled";
  error.status = null;
  return error;
}
...
export function hasPersistedBrowserLocalRuntimeConfig() {
  ...
  try {
    return [LOCAL_RUNTIME_BASE_KEY, LOCAL_EXTENSION_BASE_KEY].some((key) => {
      const value = browserWindow.localStorage.getItem(key);
...
export function hasExplicitBrowserLocalRuntimeOptIn() {
  const browserWindow = safeWindow();
  if (!browserWindow?.localStorage) return false;
  try {
    return browserWindow.localStorage.getItem(LOCAL_RUNTIME_OPT_IN_KEY) === "1";
  } catch {
    return false;
  }
}
...
export function shouldAutoProbeBrowserLocalRuntime() {
  ...
  if (!hasExplicitBrowserLocalRuntimeOptIn()) return false;
...
export function persistBrowserLocalRuntimeConfig({ runtimeBaseUrl, extensionBaseUrl, localModel } = {}) {
  ...
  if (runtimeBaseUrl || extensionBaseUrl || localModel) {
    safeWriteStorage(LOCAL_RUNTIME_OPT_IN_KEY, "1");
...
async function requestOptionalHealth(baseUrl) {
  if (!shouldAutoProbeBrowserLocalRuntime()) {
    return {
      ok: false,
      status: null,
      endpoint: joinRuntimeUrl(baseUrl, "/health"),
      payload: null,
      error: "Healthcheck local desabilitado fora do host local.",
    };
  }
...
async function requestCachedRuntimeHealth(baseUrl) {
  if (!shouldAutoProbeBrowserLocalRuntime()) {
    throw buildBrowserLocalRuntimeDisabledError("Runtime local do navegador desabilitado fora do host local.");
  }
  const endpoint = joinRuntimeUrl(baseUrl, "/health");
...
export async function hydrateBrowserLocalProviderOptions(providerOptions = []) {
  ...
  if (!shouldAutoProbeBrowserLocalRuntime()) {
    ...
    nextOptions[localIndex] = {
      ...
      disabled: true,
      available: false,
      source: localOption.source || "browser_local_runtime",
      reason: "Runtime local do navegador desabilitado neste host. Use localhost, app desktop ou override explícito.",
    };
    return nextOptions;
  }
...
export async function probeBrowserLocalStackSummary() {
  if (!shouldAutoProbeBrowserLocalRuntime()) {
    ...
    return {
      ...
      extensionHealth: {
        ok: false,
        endpoint: joinRuntimeUrl(runtimeConfig.extensionBaseUrl, "/health"),
        status: null,
        error: "Healthcheck local desabilitado fora do host local.",
      },
      localProvider: {
        ...
        available: false,
        model: runtimeConfig.localModel || DEFAULT_LOCAL_MODEL,
        baseUrl: runtimeMeta.endpoint,
        auth: null,
        runtimeFamily: null,
        runtimeLabel: "Runtime local desabilitado",
        transport: null,
        transportEndpoint: null,
        reachable: false,
        diagnosticsError: "Runtime local do navegador desabilitado neste host.",
      ...
      recommendations: [
        "Desative o provider local no navegador de produção ou use localhost/app desktop para o runtime local.",
      ],
      actions: [
...
export async function invokeBrowserLocalMessages({
  ...
  try {
    runtimeHealth = await requestCachedRuntimeHealth(runtimeBaseUrl);
  } catch (healthError) {
    lastError = healthError;
  }

  const localRuntimeHealth = runtimeHealth?.providers?.local || null;
  const shouldBypassMessagesRoute =
    localRuntimeHealth &&
    (
      localRuntimeHealth.configured === false ||
      localRuntimeHealth.available === false ||
      localRuntimeHealth.diagnostics?.reachable === false ||
      !localRuntimeHealth.diagnostics?.transport
    );

  if (shouldBypassMessagesRoute) {
    const executePayload = await invokeBrowserLocalExecute({
      ...
      context: {
        ...
        browserLocalRuntime: {
          ...
          fallback: "execute_without_messages_route",
          providerConfigured: Boolean(localRuntimeHealth?.configured),
          providerReachable: Boolean(localRuntimeHealth?.diagnostics?.reachable),
        },
      },
    });
    const text =
    ...
    return {
      ...
      data: {
        ...
        _metadata: {
          ...
          fallbackReason: "provider_not_ready",
        },
      },
    };
  }
