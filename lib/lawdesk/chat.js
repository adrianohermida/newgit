  ...
  if (primaryBaseUrl) {
    try {
      ...
      telemetry.push(createTelemetryEvent("backend_execute", {
        status: "error",
        provider: "primary_api",
        error: error?.message || "Falha no endpoint primario.",
      }));
    }
  }
  ...
  if (edgeBaseUrl) {
    try {
      ...
      telemetry.push(createTelemetryEvent("backend_execute", {
        status: "error",
        provider: "supabase_edge",
        error: error?.message || "Falha no fallback Supabase Edge Function.",
      }));
    }
  }
    ...
    if (env?.AI && typeof env.AI.run === "function") {
      try {
        ...
        telemetry.push(createTelemetryEvent("backend_execute", {
          status: "error",
          provider: "workers_ai_direct",
          error: error?.message || "Falha no Workers AI binding direto.",
        }));
      }
    } else {
  ...
  if (errors.length && env?.AI && typeof env.AI.run === "function") {
    try {
      ...
      telemetry.push(createTelemetryEvent("backend_execute", {
        ...
        provider: "workers_ai_direct",
        fallback: true,
        error: error?.message || "Falha no Workers AI binding direto (fallback).",
      }));
    }
  }
