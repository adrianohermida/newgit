...
import { routeDotobotDirectTool } from "./direct_tool_router.js";
import {
...
export async function runLawdeskChat(env, payload) {
  ...
  const effectivePromptEnhancement =
    ...
    "";
  const toolTrainingPrompt = baseContext?.repositoryContext?.toolTrainingPrompt || "";
  const finalSystemPrompt = effectivePromptEnhancement
    ? buildEnhancedSystemPrompt(baseSystemPrompt, {
        ...
        prompt: [effectivePromptEnhancement, toolTrainingPrompt].filter(Boolean).join(" "),
      })
    : toolTrainingPrompt
      ? buildEnhancedSystemPrompt(baseSystemPrompt, {
          name: "Treinamento operacional de tools",
          category: "operacional",
          prompt: toolTrainingPrompt,
        })
      : baseSystemPrompt;
  const context = {
    ...baseContext,
  ...
  try {
    const direct = await routeDotobotDirectTool(env, {
      query,
      context,
      payload,
    });
    if (direct?.handled && direct.response) {
      const memory = await persistDotobotMemory(env, {
        sessionId: direct.response.sessionId,
        query,
        responseText: direct.response.resultText,
        context: baseContext,
        status: direct.response.status,
        steps: direct.response.steps,
      });
      telemetry.push(createTelemetryEvent("backend_execute", {
        status: direct.response.status || "ok",
        provider: "direct_tool_router",
        requested_provider: executionPlan.requestedProvider,
        model: null,
        duration_ms: null,
        retries_used: 0,
    ...
    const errorMessage = error?.message || "Falha no roteador deterministico.";
    errors.push(`[direct_tool_router] ${errorMessage}`);
    telemetry.push(createTelemetryEvent("backend_execute", {
      status: "error",
      provider: "direct_tool_router",
      requested_provider: executionPlan.requestedProvider,
