...
export default function useAiTaskRuntimeHealth(props) {
  const { fallbackProviderOptions, fallbackSkillOptions, provider, pushLog, setProvider } = props;
  ...
  useEffect(() => {
    ...
    adminFetch("/api/admin-dotobot-rag-health?include_upsert=0", { method: "GET" })
      ...
      .catch((fetchError) => {
        if (active) setRagHealth({ status: "failed", error: fetchError?.message || "Falha no healthcheck RAG.", signals: {} });
      });
    return () => {
      active = false;
    };
  ...
  async function handleCopySupabaseLocalEnvBlock() {
    ...
    try {
      ...
      pushLog({ type: "control", action: "Env local copiado", result: "Bloco de variaveis do Supabase local copiado para a area de transferencia." });
    } catch {}
  }
