  ...
  return (
    <section className="border-t border-[#1B2925] bg-[linear-gradient(180deg,rgba(8,11,10,0.94),rgba(6,8,7,0.98))] p-4">
      ...
      <div className="rounded-[28px] border border-[#22342F] bg-[rgba(7,9,8,0.98)] p-3 shadow-[0_14px_40px_rgba(0,0,0,0.18)]" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); handleAttachmentDrop?.(event.dataTransfer?.files || []); }}>
        <textarea ref={missionInputRef} value={mission} onChange={(event) => handleMissionChange(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); handleStart(); } }} rows={4} placeholder="Descreva a tarefa juridica, o objetivo e os arquivos de apoio..." className="w-full resize-none bg-transparent px-2 py-2 text-sm leading-7 text-[#F5F1E8] outline-none placeholder:text-[#60706A]" />
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-3 border-t border-[#1B2925] px-2 pt-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-[11px] text-[#7F928C]">Arraste multiplos arquivos para esta area ou use Enter para enviar.</span>
          </div>
