...
export default function RunsPane({
  ...
  return (
    <aside className={`min-h-0 rounded-[28px] border p-4 shadow-[0_16px_48px_rgba(0,0,0,0.12)] ${isLightTheme ? "border-[#D7DEE8] bg-white" : "border-[#22342F] bg-[rgba(255,255,255,0.025)]"} ${className}`.trim()}>
      ...
      {historyTotalPages > 1 ? <div className={`mt-4 flex items-center justify-between gap-2 border-t pt-4 ${isLightTheme ? "border-[#E3EAF2]" : "border-[#1B2925]"}`}><button type="button" onClick={onPrevPage} disabled={historyPage <= 1} className={`rounded-full border px-3 py-1 text-[11px] transition disabled:opacity-40 ${isLightTheme ? "border-[#D7DEE8] bg-white text-[#51606B]" : "border-[#22342F] text-[#D8DEDA]"}`}>Anterior</button><span className="text-[11px] text-[#7F928C]">Pagina {historyPage} de {historyTotalPages}</span><button type="button" onClick={onNextPage} disabled={historyPage >= historyTotalPages} className={`rounded-full border px-3 py-1 text-[11px] transition disabled:opacity-40 ${isLightTheme ? "border-[#D7DEE8] bg-white text-[#51606B]" : "border-[#22342F] text-[#D8DEDA]"}`}>Proxima</button></div> : null}
    </aside>
  );
}
