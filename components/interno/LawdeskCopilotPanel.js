...
const STORAGE_PREFIX = "lawdesk_internal_copilot_v1";
...
export default function LawdeskCopilotPanel({ profile, routePath }) {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      const answerText = payload?.data?.resultText || payload?.data?.result || "Sem resposta da IA.";
      ...
      setError(submitError?.message || "Falha ao consultar a IA Lawdesk.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <header className="border-b border-[#2D2E2E] px-4 py-4">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <h3 className="font-serif text-xl">Painel Conversacional</h3>
          </div>
          ...
          <div ref={scrollRef} className="max-h-[56vh] overflow-y-auto px-4 py-4 space-y-3">
            {messages.length ? (
              messages.map((message) => (
                ...
                >
                  <p className="mb-1 text-[10px] uppercase tracking-[0.16em] opacity-60">
                    {message.role === "assistant" ? "Lawdesk AI" : "Voce"}
                  </p>
              ...
              <p className="text-sm opacity-65">
                Inicie a conversa. Historico fica salvo no navegador para este perfil administrativo.
              </p>
            )}
            {loading ? <p className="text-sm opacity-65">Gerando resposta...</p> : null}
          ...
          <div className="border-t border-[#2D2E2E] px-4 py-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-2">
              <textarea
                ...
                rows={4}
                placeholder="Pergunte para a IA Lawdesk..."
