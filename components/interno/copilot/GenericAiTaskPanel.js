...
export default function GenericAiTaskPanel(props) {
  const { activeTaskLabel, activeTaskProviderLabel, activeTaskStepCount, handleEnableNotifications, handlePause, handleResetTasks, handleRetry, isLightTheme, notificationsEnabled, onOpenAiTask, parseProviderPresentation, runningCount, taskHistory, TaskStatusChip } = props;

  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <p className={`text-sm ${isLightTheme ? "text-[#6B7C88]" : "text-[#9BAEA8]"}`}>Tarefas em andamento e próximos passos sugeridos.</p>
        <div className="flex gap-2">
          ...
          <button type="button" onClick={handleResetTasks} className={`rounded-2xl border px-3 py-2 text-xs transition ${isLightTheme ? "border-[#D7DEE8] text-[#6B7C88] hover:border-[#9A6E2D] hover:text-[#9A6E2D]" : "border-[#22342F] text-[#D8DEDA] hover:border-[#C5A059] hover:text-[#C5A059]"}`}>Limpar</button>
        </div>
      </div>
      ...
      {taskHistory.length ? taskHistory.map((task) => (
        <article key={task.id} className={`rounded-[18px] border p-4 text-sm ${isLightTheme ? "border-[#D7DEE8] bg-white" : "border-[#22342F] bg-[rgba(255,255,255,0.02)]"}`}>
          <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] uppercase tracking-[0.18em] text-[#7F928C]"><TaskStatusChip status={task.status} /></p><p className={`mt-2 line-clamp-3 font-semibold ${isLightTheme ? "text-[#152421]" : "text-[#F5F1E8]"}`}>{task.query}</p></div><span className={`text-[10px] ${isLightTheme ? "text-[#7B8B98]" : "text-[#9BAEA8]"}`}>{task.startedAt ? new Date(task.startedAt).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" }) : "--"}</span></div>
          <div className={`mt-3 flex flex-wrap gap-2 text-[11px] ${isLightTheme ? "text-[#6B7C88]" : "text-[#9BAEA8]"}`}><StatPill isLightTheme={isLightTheme}>{parseProviderPresentation(task.provider || "gpt").name}</StatPill><StatPill isLightTheme={isLightTheme}>{task.steps?.length || 0} etapas</StatPill>{task.rag?.retrieval?.enabled ? <StatPill isLightTheme={isLightTheme}>RAG {task.rag.retrieval.matches?.length || 0}</StatPill> : null}</div>
          <div className="mt-3 flex flex-wrap gap-2"><button type="button" onClick={() => handlePause(task)} className={`rounded-full border px-3 py-1.5 text-[11px] transition ${isLightTheme ? "border-[#D7DEE8] text-[#6B7C88] hover:border-[#9A6E2D] hover:text-[#9A6E2D]" : "border-[#22342F] text-[#D8DEDA] hover:border-[#C5A059] hover:text-[#C5A059]"}`}>{task.status === "paused" ? "Retomar" : "Pausar"}</button><button type="button" onClick={() => handleRetry(task)} className={`rounded-full border px-3 py-1.5 text-[11px] transition ${isLightTheme ? "border-[#D7DEE8] text-[#6B7C88] hover:border-[#9A6E2D] hover:text-[#9A6E2D]" : "border-[#22342F] text-[#D8DEDA] hover:border-[#C5A059] hover:text-[#C5A059]"}`}>Replay</button></div>
        </article>
