...
export default function FocusedConversationThread(props) {
  const {
    error,
    focusedConversationColumnClass,
    handleCopyMessage,
    handleLocalStackAction,
    handleMessageAction,
    handleOpenMessageInAiTask,
    handleReuseMessage,
    isLightTheme,
    loading,
    localInferenceAlert,
    ...
    <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-4 py-4 md:px-5">
      <div className={`flex min-h-full flex-col justify-end space-y-3 ${focusedConversationColumnClass}`}>
        {messages.length ? messages.map((message, idx) => (
          <DotobotMessageBubble key={message.id || idx} message={message} onCopy={handleCopyMessage} onReuse={handleReuseMessage} onOpenAiTask={handleOpenMessageInAiTask} onAction={handleMessageAction} />
        )) : <EmptyState isLightTheme={isLightTheme} />}
        {loading ? <DotobotMessageBubble message={{ role: "assistant", text: "", createdAt: null }} isTyping={true} /> : null}
        {localInferenceAlert && !messages.length ? <LocalAlert alert={localInferenceAlert} handleLocalStackAction={handleLocalStackAction} isLightTheme={isLightTheme} /> : null}
        {error ? (
