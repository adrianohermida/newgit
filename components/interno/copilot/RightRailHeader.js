  ...
  return (
    <button
      ...
      className={`rounded-full border px-3 py-1.5 text-[11px] transition ${
        active
          ? "border-[#C5A059] bg-[rgba(197,160,89,0.10)] text-[#9A6E2D] shadow-[0_8px_24px_rgba(197,160,89,0.10)]"
          : isLightTheme
...
export default function RightRailHeader({
  ...
  return (
    <div className={`border-b ${useCondensedRightRail ? "px-3 py-3" : "px-4 py-4"} ${isLightTheme ? "border-[#D7DEE8]" : "border-[#22342F]"}`}>
      <div className={`flex flex-col ${useCondensedRightRail ? "gap-2" : "gap-3"}`}>
        <div>
          <p className={`text-[10px] uppercase tracking-[0.2em] ${isLightTheme ? "text-[#7B8B98]" : "text-[#7F928C]"}`}>Painel lateral</p>
          <p className={`mt-1.5 font-semibold ${useCondensedRightRail ? "text-[13px]" : "text-sm"} ${isLightTheme ? "text-[#152421]" : "text-[#F5F1E8]"}`}>{activeRightPanelMeta.title}</p>
          {!useCondensedRightRail ? <p className={`mt-1 max-w-[19rem] text-xs leading-5 ${isLightTheme ? "text-[#6B7C88]" : "text-[#7F928C]"}`}>{activeRightPanelMeta.detail}</p> : null}
        </div>
        <div className={`flex flex-wrap ${useCondensedRightRail ? "gap-1.5" : "gap-2"}`}>
          {availableRightPanelTabs.includes("modules") ? <TabButton active={rightPanelTab === "modules"} isLightTheme={isLightTheme} label="Modulos" onClick={() => setRightPanelTab("modules")} /> : null}
