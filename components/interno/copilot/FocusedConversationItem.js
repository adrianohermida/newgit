...
import FocusedConversationItemActions from "./FocusedConversationItemActions";
...
export default function FocusedConversationItem({
  ...
  return (
    <article className={`group rounded-[18px] border px-3 py-3 transition-all duration-200 ease-out will-change-transform ${active ? isLightTheme ? "border-[#D2B06A] bg-[#FFF8EA] shadow-[0_12px_28px_rgba(197,160,89,0.12)] ring-1 ring-[#E6D29A]/50" : "border-[#C5A059] bg-[rgba(197,160,89,0.08)] shadow-[0_12px_28px_rgba(0,0,0,0.24)] ring-1 ring-[#C5A059]/25" : isLightTheme ? "border-transparent bg-transparent hover:-translate-y-[1px] hover:border-[#D7DEE8] hover:bg-white hover:shadow-[0_12px_24px_rgba(148,163,184,0.12)]" : "border-transparent bg-transparent hover:-translate-y-[1px] hover:border-[#22342F] hover:bg-[rgba(255,255,255,0.02)] hover:shadow-[0_12px_24px_rgba(0,0,0,0.18)]"}`}>
      <div className="flex items-start justify-between gap-3">
        <button type="button" onClick={() => onSelectConversation(conversation)} onDoubleClick={() => startInlineEdit()} className="min-w-0 flex-1 text-left transition-transform duration-200 active:scale-[0.995]">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className={`inline-flex h-2.5 w-2.5 rounded-full ${active ? "bg-[#C5A059]" : isLightTheme ? "bg-[#CBD5E1]" : "bg-[#35554B]"}`} />
                {inlineEditing ? (
                  <input value={inlineTitle} onChange={(event) => setInlineTitle(event.target.value)} onBlur={commitInlineEdit} onClick={(event) => { event.preventDefault(); event.stopPropagation(); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); commitInlineEdit(); } else if (event.key === "Escape") { event.preventDefault(); cancelInlineEdit(); } }} className={`h-7 min-w-0 flex-1 rounded-md border px-2 text-sm outline-none ${isLightTheme ? "border-[#D7DEE8] bg-white text-[#152421]" : "border-[#22342F] bg-[#0F1513] text-[#F5F1E8]"}`} />
                ) : (
        ...
        <div className={`shrink-0 transition-opacity duration-200 ${active ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
          <FocusedConversationItemActions conversation={conversation} isLightTheme={isLightTheme} onArchiveConversation={onArchiveConversation} onDeleteConversation={onDeleteConversation} onRenameConversation={onRenameConversation} onShareConversation={onShareConversation} />
        </div>
      </div>
