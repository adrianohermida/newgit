...
export class CopilotConversationRoom extends DurableObject<Env> {
  ...
  async appendMessage(message: CopilotRoomMessage): Promise<RoomSnapshot> {
  ...
  async listMessages(limit = 100): Promise<CopilotRoomMessage[]> {
    this.ctx.storage.sql.exec(
      `CREATE TABLE IF NOT EXISTS messages (
        id TEXT PRIMARY KEY,
        role TEXT NOT NULL,
  ...
  async listMessagesSince(createdAt: string, limit = 100): Promise<CopilotRoomMessage[]> {
    this.ctx.storage.sql.exec(
      `CREATE TABLE IF NOT EXISTS messages (
        id TEXT PRIMARY KEY,
        role TEXT NOT NULL,
