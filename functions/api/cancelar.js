...
// Cloudflare Pages Function para confirmaÃ§Ã£o de agendamento via link seguro
// Endpoint: /functions/api/cancelar.js
...
export async function onRequestGet(context) {
  ...
  if (!token) {
    if (wantsJson) {
      return jsonResponse(400, { ok: false, status: 'erro', message: 'Token de confirmaÃ§Ã£o ausente.' });
    }
    return new Response('Token de confirmaÃ§Ã£o ausente.', { status: 400 });
  }
  if (!/^[0-9a-fA-F-]{36}$/.test(token)) {
    if (wantsJson) {
      return jsonResponse(400, { ok: false, status: 'erro', message: 'Token de confirmaÃ§Ã£o invÃ¡lido.' });
    }
    return new Response('Token de confirmaÃ§Ã£o invÃ¡lido.', { status: 400 });
  }
  ...
  if (!supabaseUrl || !supabaseKey || supabaseKeyMeta.format === 'malformed_jwt') {
    console.error('cancelar: configuracao invalida do Supabase.', supabaseKeyMeta);
    if (wantsJson) {
  ...
  if (!resp.ok) {
    ...
    console.error('cancelar: erro ao consultar agendamento no Supabase:', detail || resp.status);
    if (wantsJson) {
  ...
  if (!agendamentos.length) {
    if (wantsJson) {
      return jsonResponse(404, { ok: false, status: 'erro', message: 'Token invÃ¡lido ou agendamento nÃ£o encontrado.' });
    }
    return new Response('Token invÃ¡lido ou agendamento nÃ£o encontrado.', { status: 404 });
  }
  ...
  if (agendamento.status === 'confirmado') {
    ...
    if (wantsJson) {
      return jsonResponse(200, {
        ...
        message: 'Este agendamento jÃ¡ foi confirmado anteriormente.',
        confirmedAt: agendamento.confirmed_at || null,
        confirmedLabel,
      });
    }
    const htmlJaConfirmado = `<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>JÃ¡ Confirmado | Hermida Maia</title><style>body{margin:0;font-family:sans-serif;background:#050706;color:#F4F1EA;display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center}.card{background:#111;border:1px solid #2D2E2E;border-radius:12px;padding:48px 32px;max-width:480px}.icon{font-size:48px;margin-bottom:16px}.title{color:#C5A059;font-size:24px;font-weight:bold;margin-bottom:12px}.btn{display:inline-block;background:#C5A059;color:#050706;font-weight:bold;padding:12px 28px;border-radius:8px;text-decoration:none;margin-top:16px}.meta{font-size:13px;color:#aaa;margin-top:12px}</style></head><body><div class="card"><div class="icon">â„¹ï¸</div><div class="title">Agendamento jÃ¡ confirmado</div><p>Este agendamento jÃ¡ foi confirmado anteriormente.</p>${confirmedLabel ? `<p class="meta">Confirmado em ${confirmedLabel}</p>` : ''}<a class="btn" href="https://hermidamaia.adv.br">Voltar ao site</a></div></body></html>`;
    return new Response(htmlJaConfirmado, { status: 200, headers: { 'Content-Type': 'text/html; charset=UTF-8' } });
  }
  // ExpiraÃ§Ã£o do token: 24h apÃ³s criaÃ§Ã£o
  const criadoEm = new Date(agendamento.created_at);
  const agora = new Date();
  const expirado = (agora - criadoEm) > 24 * 60 * 60 * 1000;
  if (expirado) {
    if (wantsJson) {
      return jsonResponse(410, { ok: false, status: 'expirado', message: 'Este link de confirmaÃ§Ã£o expirou. FaÃ§a um novo agendamento.' });
    }
    const htmlExpirado = `<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><title>Link Expirado | Hermida Maia</title><style>body{margin:0;font-family:sans-serif;background:#050706;color:#F4F1EA;display:flex;align-items:center;justify-content:center;min-height:100vh;text-align:center}.card{background:#111;border:1px solid #2D2E2E;border-radius:12px;padding:48px 32px;max-width:480px}.icon{font-size:48px;margin-bottom:16px}.title{color:#C5A059;font-size:24px;font-weight:bold;margin-bottom:12px}.btn{display:inline-block;background:#C5A059;color:#050706;font-weight:bold;padding:12px 28px;border-radius:8px;text-decoration:none;margin-top:16px}</style></head><body><div class="card"><div class="icon">â°</div><div class="title">Link expirado</div><p>Este link de confirmaÃ§Ã£o expirou (vÃ¡lido por 24h). FaÃ§a um novo agendamento.</p><a class="btn" href="https://hermidamaia.adv.br/agendamento">Agendar novamente</a></div></body></html>`;
  ...
  if (!updateResp.ok) {
    ...
    console.error('cancelar: erro ao atualizar status no Supabase:', detail || updateResp.status);
    if (wantsJson) {
      return jsonResponse(500, { ok: false, status: 'erro', message: 'Erro ao cancelar agendamento.' });
    }
    return new Response('Erro ao cancelar agendamento.', { status: 500 });
  }
  const updatedRows = await updateResp.json().catch(() => []);
  if (!Array.isArray(updatedRows) || updatedRows.length === 0) {
    console.error('cancelar: atualizaÃ§Ã£o sem linhas retornadas para o token:', token);
    if (wantsJson) {
      return jsonResponse(500, { ok: false, status: 'erro', message: 'Erro ao cancelar agendamento.' });
    }
    return new Response('Erro ao cancelar agendamento.', { status: 500 });
  ...
  // Montar e-mails de confirmaÃ§Ã£o
...
<div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#050706;color:#F4F1EA;padding:32px;border-radius:12px">
  ...
  <p>OlÃ¡, <strong>${agendamento.nome}</strong>!</p>
  <p>Sua consulta jurÃ­dica foi confirmada com sucesso. Aguardamos vocÃª no dia e horÃ¡rio abaixo.</p>
  <table style="width:100%;border-collapse:collapse;margin:24px 0">
    <tr><td style="padding:8px;color:#C5A059;font-weight:bold">Ãrea</td><td style="padding:8px">${agendamento.area}</td></tr>
    <tr><td style="padding:8px;color:#C5A059;font-weight:bold">Data</td><td style="padding:8px">${dataFormatada}</td></tr>
    <tr><td style="padding:8px;color:#C5A059;font-weight:bold">HorÃ¡rio</td><td style="padding:8px">${agendamento.hora}</td></tr>
    ${agendamentoConfirmado.zoom_join_url ? `<tr><td style="padding:8px;color:#C5A059;font-weight:bold">Sala virtual</td><td style="padding:8px"><a href="${agendamentoConfirmado.zoom_join_url}" style="color:#C5A059">Entrar na reuniÃ£o do Zoom</a></td></tr>` : ''}
  </table>
  ...
  <p style="font-size:13px;color:#aaa">Em caso de dÃºvidas, acesse <a href="https://hermidamaia.adv.br" style="color:#C5A059">hermidamaia.adv.br</a> ou entre em contato conosco.</p>
</div>`;

  const emailEscritorioHtml = `
<div style="font-family:sans-serif;max-width:600px;margin:0 auto">
  <h2>Agendamento Confirmado pelo Cliente â€” ${agendamento.area}</h2>
  <table style="border-collapse:collapse;width:100%">
    ...
    <tr><td style="padding:6px;font-weight:bold">Ãrea</td><td style="padding:6px">${agendamento.area}</td></tr>
    ...
    <tr><td style="padding:6px;font-weight:bold">ObservaÃ§Ãµes</td><td style="padding:6px">${agendamento.observacoes || 'â€”'}</td></tr>
    <tr><td style="padding:6px;font-weight:bold">Google Event ID</td><td style="padding:6px;font-size:12px">${agendamento.google_event_id || 'â€”'}</td></tr>
